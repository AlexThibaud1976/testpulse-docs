...
TestPulse parcourt l'arbre en **lecture seule** : plan → suites → cas → points → derniers résultats. Il ne modifie des données qu'il n'a pas créées que dans deux actions explicitement confirmées — la réaffectation en masse et la décision de passage (voir [ce que TestPulse écrit](/fr/concepts/read-only)).
