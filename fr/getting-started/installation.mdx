...
TestPulse demande des scopes de lecture, plus deux scopes d'écriture, chacun borné à une action de création que vous déclenchez vous-même :
...
| `vso.work_write` | **Test exploratoire uniquement** — créer les bugs, cas de test et la tâche de session que vous déposez, et leurs pièces jointes. |
| `vso.build`, `vso.identity` | Détails du build associé et des assignés. |

<Tip>TestPulse n'écrit jamais de lui-même : chaque écriture suit un geste explicite. En dehors de deux actions explicitement confirmées — la réaffectation en masse et la décision de passage — il ne modifie jamais un work item, plan, suite ou résultat antérieur à ce geste — voir [Permissions & scopes](/fr/security/permissions).</Tip>