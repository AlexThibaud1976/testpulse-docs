...
<Note>A second source — seeding from the plan/suite hierarchy — was removed in **v2.61.0**, and the source picker went with it: a single remaining option had nothing left to choose, so the dialog now opens straight on the counted preview. On a real, plan-heavy project it produced a tree several times more crowded than the same project's Area Paths, from far more requests — not through an implementation defect, but because a tree with **one parent per folder** cannot structurally mirror plans that share their test cases. Anything you already seeded from it is untouched.</Note>
