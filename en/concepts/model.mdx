...
TestPulse walks the tree **read-only**: plan → suites → cases → points → latest results. It never modifies anything it did not create itself (see [what TestPulse writes](/en/concepts/read-only)).
