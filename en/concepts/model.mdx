...
TestPulse walks the tree **read-only**: plan → suites → cases → points → latest results. It modifies data it did not create in only two explicitly confirmed actions — bulk reassignment and the promotion decision (see [what TestPulse writes](/en/concepts/read-only)).
