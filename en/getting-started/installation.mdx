...
TestPulse requests read scopes plus two write scopes, each bounded to a create action you trigger yourself:
...
| `vso.work_write` | **Exploratory testing only** — create the bugs, test cases and session Task you file, and their attachments. |
| `vso.build`, `vso.identity` | Associated build and assignee details. |

<Tip>TestPulse never writes on its own: every write follows an explicit gesture. Outside two explicitly confirmed actions — bulk reassignment and the promotion decision — it never modifies a work item, plan, suite or result that existed before that gesture — see [Permissions & scopes](/en/security/permissions).</Tip>