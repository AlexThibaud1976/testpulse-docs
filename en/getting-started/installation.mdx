...
<Tip>TestPulse never writes on its own: every write follows an explicit gesture. It never modifies a work item, plan, suite or result that existed before that gesture — see [Permissions & scopes](/en/security/permissions).</Tip>